...
PROCESSOR_UNHEALTHY_GRACE_SECONDS=120 # optional, grace window (seconds) before a timed-out processor flips to unhealthy; 0 disables (default 120)
...
WEBAPP_HOST=127.0.0.1      # optional, bind host for the Mini App server (default 127.0.0.1; compose.yaml overrides to 0.0.0.0 so Caddy can reach the bot over the docker network)
