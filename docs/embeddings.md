...
| --- | --- | --- | --- |
| `EMBEDDING_PROVIDER` | `auto` \| `openai` \| `ollama` \| `remote` \| `none` | `auto` | `auto` picks `openai` if `OPENAI_API_KEY` is set, otherwise `none`. `remote` routes through `processor_bot` — see [processor-bot.md](processor-bot.md). |
...
- `remote` — sends the request to `processor_bot` over a private Telegram bus
  ...
  [processor-bot.md](processor-bot.md) for setup and the
  wire protocol.
